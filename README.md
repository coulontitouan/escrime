...
## Arthur GOUDAL
Mon rôle au sein de l'équipe a été le développement back-end. Conception de la base de données avec Noam DOUCET, fonctions de peuplement et de sauvegarde à partir des fichiers CSV fournis par la Fédération Française d'Escrime, algorithmes de génération de compétitions et organisation générale des fichiers ont été mes principale missions. J'ai finalement travaillé sur peu d'aspects différents de l'application, mais ces points sont critiques et nécessitent beaucoup d'investissement et de réflexion, notamment afin d'obtenir une application complètement modulable capable d'intéragir avec tous les fichiers de la Fédération et de créer des compétitions avec un nombre quelconque de participants et respectant toutes les normes d'organisation des compétitions d'escrime. Pour ce faire, j'ai souvent occupé le rôle de lien entre le professeur référent M. LECHOPIER et l'équipe afin de connaître précisément les besoins client et toutes les règles à respecter en escrime mais également en ma qualité de développeur back-end connaissant l'avancement global de l'application. 
Ce projet m'a donc permis de surtout progresser sur ma qualité de communication au sein d'une équipe et avec un client. J'ai apprécié ce rôle d'intermédiaire qui m'a permis de m'immerger encore plus dans le projet, d'y prendre plus de plaisir et de m'investir davantage. Techniquement, j'ai découvert le framework SQLAlchemy très utile en Python et j'ai progressé sur l'organisation et la conduite générale d'un projet.
